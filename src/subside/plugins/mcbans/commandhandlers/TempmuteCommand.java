...
public class TempmuteCommand extends AbstractCommand {
    ...
    public void execute(CommandSender sender, String[] args) throws Exception {
        ...
        if (args.length > 2 || (args.length > 1 && Perm.Mute.REASONOPTIONAL.has(sender))) {
            ...
            Bukkit.getScheduler().runTaskAsynchronously(mcBans.getPlugin(), new AddEventRunnable(args[0], sender, args[1], reason, Event.MUTE));
        } else {
            throw new MissingArgumentsException("/mute <username> <time> <reason>");
        }
    }

    @Override
    public IPerm getPerm() {
        return Perm.Mute.TEMPMUTE;
    }
